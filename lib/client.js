...
const normalize = (cmd, response) => {
  ...
  if (isArray(response.response)) {
    ...
    filteredResponse = [response.response].filter((responseCode) => responseCode.substr(0, 2).toLowerCase() === cmdType); // eslint-disable-line
  ...
  if (matched) {
    return assign(response, { response: filteredResponse, $original: response.response, $formatted: `${matched.label}: ${matched.value}` }); // eslint-disable-line
  }
...
module.exports = class DenonClient extends EventEmitter {
  ...
  send(cmd, callback) {
    return this.exec(cmd).then((response) => {
      ...
      this.emit(LIFECYCLE_CODES.RESPONSE, { code: LIFECYCLE_CODES.RESPONSE, response: normalizedResponse.response });
  ...
  exec(cmd) {
    ...
    return this.connection.send(cmd, { timeout: this.config.telnetTimeout }).then((response) => {
