...
const normalize = (response) => {
  ...
  let matched;
  const firstResponse = isArray(response.response) ? response.response[0] : response.response;
  ...
  if (matched) {
    return merge(response, { formatted: `${matched.label}: ${matched.value}` });
  }
  return response;
};
...
module.exports = class DenonClient extends EventEmitter {
  ...
  push(cmd, callback = DEFAULT_CALLBACK) {
    if (!this.queue.length && !this.tickInterval) {
      ...
      if (shouldDisconnect) {
        ...
        return this.disconnect().then(() =>
          this.emit(LIFECYCLE_CODES.DISCONNECT, {
            ...
            message: `Disconnected from host ${this.config.host}`
          })
        );
      }
    ...
    return new Promise((resolve, reject) => {
      this.connection.on('connect', () => {
        this.emit(LIFECYCLE_CODES.CONNECT, {
          ...
          message: `Connected to ${this.config.host}`
        });
        resolve();
      });
  ...
  exec(cmd) {
    ...
    this.sending = true;
    return this.connection.send(cmd, { timeout: 500 }).then((response) => {
